...
// The Secured Notes folder tree, in the SIDEBAR under Shortcuts (Jason 08-11-2026: "move the
// folders under the shortcuts sidebar"). The first mockup drew it as a third column beside the note
// list — wrong twice over: Passwords already puts its rail in the sidebar, and a third column would
// have left the editor about 200 pixels wide at the 740 floor.
//
...
import { vaultApi, type VaultNoteFolder } from "./vaultApi";
...
export default function NoteFolderRail({ selected, onSelect, reloadKey, onChanged, open: openIds, onOpen }: NoteFolderRailProps) {
  const api = vaultApi();
  const [folders, setFolders] = useState<VaultNoteFolder[]>([]);
  ...
  const [renaming, setRenaming] = useState<VaultNoteFolder | null>(null);
  const [ask, setAsk] = useState<VaultNoteFolder | null>(null);
  ...
  const [askEmpty, setAskEmpty] = useState<VaultNoteFolder | null>(null);
  ...
  const kids = useMemo(() => {
    const m = new Map<number | null, VaultNoteFolder[]>();
   ...
   * The Passwords rail has had both since it shipped (FolderMenu.tsx sets effectAllowed and
   * dropEffect; .vault-railrow.drop lights the row); this tree simply never got them.
   */
  const [over, setOver] = useState<number | null | "none">("none");

  const dragOver = (folderId: number | null, e: React.DragEvent): void => {
    if (!e.dataTransfer.types.includes("text/vault-note")) return;
    e.preventDefault();
  ...
  const dropNote = (folderId: number | null, e: React.DragEvent): void => {
    e.preventDefault();
    setOver("none");
    const uuid = e.dataTransfer.getData("text/vault-note");
    if (!uuid) return;
    void api.setNoteFolder(uuid, folderId)
      .then(() => {
        ...
        // why a moved note sat in Unfiled until Ctrl+R (08-12-2026). Fixed in VaultModule/NotesView.
        load();
        onChanged();
      })
  ...
  const row = (f: VaultNoteFolder, depth: number): React.ReactNode => {
    ...
    return (
      <div key={f.id}>
        <div
          ...
          className={`vault-frow${selected === f.id ? " on" : ""}${over === f.id ? " drop" : ""}`}
        ...
        >
          <button
            className="vault-fcar"
            title={children.length === 0 ? "" : isOpen ? "Collapse" : "Expand"}
            onClick={(e) => { e.stopPropagation(); if (children.length) toggle(f.id); }}
          >
            {children.length === 0 ? "" : isOpen ? "▾" : "▸"}
          </button>
          <span className="vault-fic" aria-hidden="true">📁</span>
          <span className="vault-fname">{f.name}</span>
          <span className="vault-fn">{counts[f.id] ?? 0}</span>
        ...
        {adding === f.id && (
          <input
            className="vault-fnew" autoFocus value={name} placeholder="Folder name"
        ...
        {renaming?.id === f.id && (
          <input
            className="vault-fnew" autoFocus value={name}
            style={{ marginLeft: 24 + depth * 14 }}
            onChange={(e) => setName(e.target.value)}
  ...
  return (
    <>
      <div className="vault-railhead">
        ...
        <button className="vault-addsc sbtxt" title="New top-level folder" onClick={() => { setName(""); setAdding("root"); }}>+ New folder</button>
      </div>

      {error && <div className="vault-hint sbtxt" style={{ padding: "2px 9px 6px", color: "var(--vault-danger-color)" }}>{error}</div>}
      {outcome && (
        <div className="vault-hint sbtxt" style={{ padding: "4px 9px 8px", color: "var(--vault-strong-color)" }}>
          {outcome}{" "}
      ...
      {unfiled > 0 && (
      <div
        className={`vault-frow${selected === -1 ? " on" : ""}${over === null ? " drop" : ""}`}
        onClick={() => onSelect(-1)}
        onDragOver={(e) => dragOver(null, e)}
        onDragLeave={() => setOver((p) => (p === null ? "none" : p))}
        onDrop={(e) => dropNote(null, e)}
      >
        <span className="vault-fcar" />
        <span className="vault-fic" aria-hidden="true">📥</span>
        <span className="vault-fname sbtxt" style={{ color: "var(--mc-muted)" }}>Unfiled</span>
        <span className="vault-fn">{unfiled}</span>
      </div>
      )}
