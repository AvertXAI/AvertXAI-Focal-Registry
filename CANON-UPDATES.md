...
## [CONTRADICTS] 2026-07-20 — autoDownload REVERSED to FALSE (consent-first); supersedes both TRUE entries above
**Canon says:** "**`autoDownload` defaults TRUE in this product only.** Paul never thinks about updating." (FR-DECISIONS-1.md §Auto-update). The two 2026-07-20 entries above ("resolved to TRUE" and "toasts at download-complete") are now **SUPERSEDED**.
**Reality (ruled by Jason 2026-07-20, reversing the earlier same-day TRUE):** `autoDownload = false` — consent-first, matching CLAUDE.md §3.12 as originally written and every other AvertXAI application. An automatic check that FINDS an update now raises a non-dismissing "Version X available" toast with a **Download** button; Download → progress → the non-dismissing "ready to restart" toast. Automatic failures stay silent; manual checks always answer. **The reason (so it is never reopened):** alpha builds are UNSIGNED by design, so electron-updater validates only a SHA512 the FEED supplies — an attacker controlling the feed controls installer AND hash, so auto-download = code execution with no click. Consent is the last human gate.
**Evidence:** `electron/core/updater.ts` (`autoUpdater.autoDownload = false`, header + inline comment give the reason); `src/App.tsx` `UpdateToast` (subscribes `updater:available`/`progress`/`downloaded`; "available" has a Download button and does not auto-dismiss; only "you're on the latest" auto-dismisses); `CLAUDE.md` §3.12 (consent-first + the security reason). Commit on branch `feature/isobmff-geometry`.
**Suggestion:** amend FR-DECISIONS §Auto-update to "**autoDownload FALSE — consent-first, matching every other AvertXAI app.** An automatic check that finds an update raises a Download-button toast; unsigned alpha builds make consent the last gate against a hostile feed." Drop the "autoDownload defaults TRUE in this product only" line.
**Severity:** worth-fixing (FR-DECISIONS still says TRUE; code + CLAUDE.md now say FALSE)

## [GAP] 2026-07-20 — scan now excludes build/dependency dirs and content-sniffs .mts
**Canon says:** FR-DECISIONS §Scan — "Reports on drives … Stills, video, and audio equally"; nothing about excluding build/dependency directories, and nothing about extensions that are ambiguous by name.
**Reality:** the scan walk now (a) skips ten build/dependency directory names at any depth (`node_modules · .git · dist · build · release · release-new · win-unpacked · .next · .cache · vendor`) — never descended, never an error row — and (b) content-sniffs `.mts` (both an AVCHD MPEG transport stream AND TypeScript's module extension): only a real transport stream is video; a TypeScript `.mts` is counted and skipped. VERIFIED first: on D:\, all 31,604 `.mts` are TypeScript (31,568 under node_modules; 0 of 24 files >1 MB pass the sync test). Predicted D:\ rescan: media 36,407→19,926, video 16,041→239. The exclusion list + sniff live in `scan/media.ts` beside the media extension set; a future Scan-settings surface would merge user edits over these defaults.
**Evidence:** `electron/core/services/scan/media.ts` (`EXCLUDED_DIR_NAMES`, `CONTENT_SNIFF_EXTS`, `isMpegTransportStream`); `electron/core/services/scan/index.ts` (`dirSkipRule`, `isRealTransportStream`, both walks); security-adjacent audit + REPORT-export-polish-2026-07-20.md.
**Suggestion:** record in DECISIONS §Scan that the walk excludes build/dependency dirs by name (list beside the media set) and classifies ambiguous extensions (`.mts`) by content, not name.
**Severity:** worth-fixing (behaviour is shipped + proven; canon should reflect it)

## [BETTER] 2026-07-20 — RB-UPDATES-01 §7.2 release script's first-match find() uploads the WRONG artifacts
**Canon says:** RB-UPDATES-01-coolify-update-server.md §7.2 selects release artifacts by first extension match: `const exe = files.find(f => f.endsWith(".exe"))`, `.blockmap`, `.yml` — then scp's payload before manifest.
**Reality:** first-match is unsafe in any repo that (a) builds a **portable** target — `build.win.target` here is `[nsis, portable]`, so every build emits a **second** `.exe` (`Portable-${version}.exe`); (b) uses electron-builder, which **always** writes **`builder-debug.yml`** beside `prerelease.yml` (two `.yml`); or (c) does **not clean** `release/` between builds (electron-builder overwrites same-named files but leaves stale ones). `readdirSync().find()` then returns whichever sorts first — the wrong file. Verified against this repo's actual `release/`: it resolved `exe → AvertXAI-Focal-Registry-Portable-0.1.0.exe`, `map → …Setup-0.1.0.exe.blockmap`, `yml → builder-debug.yml` — a stale portable exe, a mismatched 0.1.0 blockmap, and electron-builder's debug file **in place of the update manifest**. Run as-is it would scp garbage and the feed-verify would 404.
**Evidence:** `node -e "const f=require('node:fs').readdirSync('release'); …find(x=>x.endsWith('.exe'/'.blockmap'/'.yml'))"` → the three wrong names above (release/ contained Setup+Portable at 0.1.0/0.1.1 + builder-debug.yml + prerelease.yml); `package.json` `build.win.target=[nsis,portable]`. Corrected form committed here: `scripts/release.mjs` (`21225e0`) pins `AvertXAI-Focal-Registry-Setup-${version}.exe`, `${exe}.blockmap`, `prerelease.yml` and asserts each `existsSync` in `release/` before any scp (refuses a partial release). Re-verified: resolves to `Setup-0.2.0.exe` / its blockmap / `prerelease.yml`.
**Suggestion:** replace §7.2's three `find()` lines with name-pinned selection + an existence assert (as in `scripts/release.mjs` here). first-match is only safe for a single-target, single-yml, always-clean output dir — which no portable-shipping repo is.
**Severity:** worth-fixing (the verbatim runbook script would publish a broken or garbage release; Jason owns RB-UPDATES-01)

## [CONTRADICTS] 2026-07-20 — Rename "rename from the database" conflicts with the approved spec+mockup (walk source folders)
**Canon says:** FR-DECISIONS-1 §Rename — "Scan first → report to the source root → **rename from the database. Cannot rename what was never recorded.**" and §Storage — "Rename **appends to rows Scan wrote** … Rename does a **SELECT**."
**Reality:** the approved `MOCKUP-rename-module-3.html` and the Rename build spec (2026-07-20) define Rename as operating on **source folders chosen directly**, independent of Scan: preview is a pure function of *source folders + settings*, and the engine *reads the source, copies to the destination*. There is no scan prerequisite, no `scan_files` SELECT, and no run/drive picker in the mockup. A batch has MANY source folders (`rename_sources`), each with its own `source_path`/`copy_path` reversal rows in `rename_files`.
**Evidence:** spec Phases 3.1/3.5 + schema 2.2/2.3; `MOCKUP-rename-module-3.html` (source folders with add/remove, destination, per-class 001, purple Revert tab — grepped, no drive/run selector). Contrast FR-DECISIONS-1 §Rename lines quoted above.
**Suggestion:** update §Rename to "Rename copies from **user-chosen source folders** (Scan not a prerequisite); the append-only `rename_files` log (both filenames per row) is the reversal mechanism." Keep the copy-only / never-touch-original law verbatim.
**Severity:** blocking (decides the schema and engine architecture; awaiting Jason's ruling before Phase 2)

## [CONTRADICTS] 2026-07-20 — Rename "one folder at a time" conflicts with the spec's multiple source folders
**Canon says:** FR-DECISIONS-1 §Rename — "**One folder at a time, recursing into its subfolders. NEVER a drive root** — enforced as a guard, not a convention."
**Reality:** the spec+mockup support **multiple** source folders per batch (`rename_sources` is one-to-many; the mockup has add/remove rows). The "NEVER a drive root" half is compatible and should survive as a hard guard on each source.
**Evidence:** spec 2.2 + 5.1; `MOCKUP-rename-module-3.html` add/remove rows; FR-DECISIONS-1 §Rename line quoted above.
**Suggestion:** amend §Rename to "**one or more** source folders, each recursing into its subfolders, **never a drive root (enforced guard)**." The drive-root guard is retained verbatim.
**Severity:** worth-fixing (schema already models many folders; the retained guard is the real requirement)

## [GAP] 2026-07-21 — §3.4 native-overlay dim is for FULL-WINDOW modals only; content-area overlays must NOT dim
**Canon says:** §3.4 / §4.5 — "To make [native buttons] recede, *dim* them: recolor the overlay to match the modal backdrop, restore to the active theme on close." Canon presents overlay-dim as the standard treatment for any renderer modal, with no scope qualifier.
**Reality:** the dim (`setOverlayDim` → `blendWithBackdrop`, `electron/core/windows.ts:86-91`) blends the native strip **toward a fixed dark scrim** `rgba(4,8,16,.66)`. That is only correct when the DOM modal is a translucent dark scrim covering the WHOLE window (topbar included). A **content-area-only** overlay — `position:absolute; inset:0` inside the content region, below the topbar, opaque `var(--mc-base)` — never covers the native buttons, and in **light theme** dimming paints a visible **dark block** over them (the recurring Secure Note "loading modal over native buttons" defect). Rule: dim **iff** the modal's own backdrop actually overlaps the caption zone AND is the dark `.overlay` scrim. An opaque, theme-colored, content-only overlay leaves the native strip on the ACTIVE theme.
**Evidence:** `blendWithBackdrop` mixes 66% toward `#040810` (`windows.ts:86-91`); `.rbs-loadmodal{position:absolute;inset:0;background:var(--mc-base)}` (`runbook-shredder.css`) sits below the ~58px topbar, so the native buttons are never under it (screenshot: light topbar with a dark block only where the dim landed). Fix committed here: removed the `setModalDim(ingesting)` effect in `RunbookShredderModule.tsx` — native buttons stay light and match the topbar.
**Suggestion:** §3.4 should read "dim the native overlay **only when the modal's backdrop is the dark `.overlay` scrim covering the full window including the caption zone**. A content-area-only or opaque theme-colored overlay does NOT dim — dimming blends the strip dark and, in light theme, paints a dark block over the buttons."
**Severity:** worth-fixing (recurring — this defect has been reintroduced repeatedly for lack of the scope qualifier)
