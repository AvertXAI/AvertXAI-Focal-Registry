...
export default function EntriesView({ secrets, folders = [], loading, error, filter, search, onReload, onNew, onEdit }: EntriesViewProps) {
  const api = vaultApi();
  ...
  const copy = useCallback(
    (uuid: string): void => {
      ...
      void api
        .read(uuid)
        .then((full) => api.copyText(full.value))
        .catch((e: unknown) => setRowError(e instanceof Error ? e.message : String(e)))
        .finally(() => setBusy(null));
    },
    [api]
...
export function EntryModal({
  ...
  const save = (): void => {
    ...
    // An existing entry: metadata always, credential only when one was typed. The public key is
    // metadata (the form shows the stored one, unlike the password) — omitting it here was Tier-1
    // fix 4: edits saved on create and were silently discarded on every edit after that.
    void api
      .updateMeta(secret.uuid, {
        ...
        url: form.url,
        notes: form.notes,
        ...(isSsh ? { publicKey: publicKey.trim() } : {}),
      })
