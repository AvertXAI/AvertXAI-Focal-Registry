...
// Description: The MindMerge authored-document folder tree — the Secured Notes tree copied across
//              intact (RULED by Jason 08-21-2026). Jason ruled all three shape questions on
//              08-11-2026, when this tree was born in the vault:
...
//              · ONE folder per document. Tags do the many-places job better, and a document in two
//                places makes "delete this folder" ambiguous.
...
//                the same thing.
//
//              This tree governs the AUTHORING tables only (mindmerge_docs / mindmerge_doc_folders).
//              The INGEST tables (mindmerge_notes, indexing markdown found on disk) are a different
//              shape with their own life and are untouched by anything in this file.
//
//              DELETING A FOLDER DELETES EVERYTHING IN IT. Subfolders, and every document in any of
...
//                · deleteNoteFolder  — the folder, its subfolders, and every document. Gone.
//                · archiveNoteFolder — every document to the Archived shelf, folders removed. Restorable.
//                    (Jason 08-12-2026: the folder prompt should offer the same three choices the
//                     note trashbin does — cancel, delete, archive.)
//                · emptyNoteFolder   — every document to Unfiled, folders left standing. Nothing deleted.
//
//              Electron-free and filesystem-free, like every other document service: this file cannot
//              touch a file on disk even by accident.
// License: Proprietary / Unauthorized copying of this file is strictly prohibited
// File: electron/core/services/mindmerge/noteFolders.ts
//------------------------------------------------------------
import { generateUUIDv7, nowIso, type Db } from "./db";
...
export function listNoteFolders(db: Db, orgId: string): MindMergeDocFolder[] {
  return db
    .prepare("SELECT id, uuid, name, parent_id, sort_order FROM mindmerge_doc_folders WHERE org_id = ? ORDER BY sort_order, name")
    .all(orgId) as MindMergeDocFolder[];
}

/**
 * Document counts per folder, INCLUSIVE OF DESCENDANTS.
 *
 * A parent reading 0 while its children hold a thousand documents is the kind of number that makes
...
export function noteFolderCounts(db: Db, orgId: string): Record<number, number> {
  const direct = db
    .prepare("SELECT folder_id AS id, COUNT(*) AS n FROM mindmerge_docs WHERE org_id = ? AND folder_id IS NOT NULL AND archived_at IS NULL GROUP BY folder_id")
  ...
  for (const row of direct) {
    ...
    let hops = 0;
    // Walk UP, adding this folder's own documents to every ancestor. The hop guard means a cycle
...
/** How many documents have no folder at all. Unfiled is always shown, even at zero — a hidden Unfiled
    is how documents get lost. */
export function unfiledNoteCount(db: Db, orgId: string): number {
  return (db
    .prepare("SELECT COUNT(*) AS n FROM mindmerge_docs WHERE org_id = ? AND folder_id IS NULL AND archived_at IS NULL")
    .get(orgId) as { n: number }).n;
}

export function createNoteFolder(db: Db, orgId: string, name: unknown, parentId?: unknown): MindMergeDocFolder {
  const clean = cleanName(name);
  const parent = parentId == null || parentId === "" ? null : Number(parentId);
  if (parent != null) {
    const exists = db.prepare("SELECT id FROM mindmerge_doc_folders WHERE id = ? AND org_id = ?").get(parent, orgId);
    if (!exists) throw new Error("That parent folder does not exist.");
  }
  const at = nowIso();
  const info = db
    .prepare("INSERT INTO mindmerge_doc_folders (uuid, org_id, name, parent_id, sort_order, created_at) VALUES (?, ?, ?, ?, 0, ?)")
    .run(generateUUIDv7(), orgId, clean, parent, at);
  return db
    .prepare("SELECT id, uuid, name, parent_id, sort_order FROM mindmerge_doc_folders WHERE id = ?")
    .get(Number(info.lastInsertRowid)) as MindMergeDocFolder;
}

export function renameNoteFolder(db: Db, orgId: string, id: unknown, name: unknown): MindMergeDocFolder {
  const clean = cleanName(name);
  const folderId = Number(id);
  const changed = db
    .prepare("UPDATE mindmerge_doc_folders SET name = ?, updated_at = ? WHERE id = ? AND org_id = ?")
    .run(clean, nowIso(), folderId, orgId).changes;
  if (changed === 0) throw new Error("That folder does not exist.");
  return db
    .prepare("SELECT id, uuid, name, parent_id, sort_order FROM mindmerge_doc_folders WHERE id = ?")
    .get(folderId) as MindMergeDocFolder;
}
...
export function moveNoteFolder(db: Db, orgId: string, id: unknown, newParentId: unknown): MindMergeDocFolder {
  const folderId = Number(id);
  ...
  const changed = db
    .prepare("UPDATE mindmerge_doc_folders SET parent_id = ?, updated_at = ? WHERE id = ? AND org_id = ?")
 ...
 *   TRUE  — the folder, every folder beneath it, and every document in any of them. Jason's default.
 *   FALSE — the folder only; its documents go to Unfiled and its children lift one level.
...
export function deleteNoteFolder(
  ...
  const folderId = Number(id);
  const row = db.prepare("SELECT parent_id FROM mindmerge_doc_folders WHERE id = ? AND org_id = ?").get(folderId, orgId) as
    | { parent_id: number | null }
    | undefined;
  ...
  db.transaction(() => {
    deletedNotes = db.prepare(`DELETE FROM mindmerge_docs WHERE org_id = ? AND folder_id IN (${marks})`).run(orgId, ...ids).changes;
    deletedFolders = db.prepare(`DELETE FROM mindmerge_doc_folders WHERE org_id = ? AND id IN (${marks})`).run(orgId, ...ids).changes;
  })();
 ...
 *
 * It is the document-level archive applied to a whole subtree: every document in it moves to the
 * Archived shelf with its text kept in full and restorable, and the now-empty folders are removed. The
 * folders go because leaving a tree of empty boxes behind is not "archived", it is litter — and the
 * documents are still reachable on the Archived shelf, which is the point of the shelf existing.
 *
 * Already-archived documents are left alone rather than re-stamped, so the returned count is what this
...
export function archiveNoteFolder(
  ...
  const folderId = Number(id);
  const exists = db.prepare("SELECT id FROM mindmerge_doc_folders WHERE id = ? AND org_id = ?").get(folderId, orgId);
  ...
  db.transaction(() => {
    archivedNotes = db
      .prepare(
        `UPDATE mindmerge_docs SET archived_at = ?, updated_at = ?, folder_id = NULL
    ...
    db.prepare(`UPDATE mindmerge_docs SET folder_id = NULL WHERE org_id = ? AND folder_id IN (${marks})`).run(orgId, ...ids);
    deletedFolders = db.prepare(`DELETE FROM mindmerge_doc_folders WHERE org_id = ? AND id IN (${marks})`).run(orgId, ...ids).changes;
  })();
...
/**
 * EMPTY a folder — keep every document, just stop them being filed here. Jason asked for it as a
 ...
 * button.
 *
 * Moves the WHOLE SUBTREE's documents, not just the ones sitting directly in this folder. The earlier
...
export function emptyNoteFolder(db: Db, orgId: string, id: unknown): { movedNotes: number } {
  ...
  const marks = ids.map(() => "?").join(",");
  const movedNotes = db
    .prepare(`UPDATE mindmerge_docs SET folder_id = NULL, updated_at = ? WHERE org_id = ? AND folder_id IN (${marks})`)
...
/** Move ONE document into a folder, or to Unfiled with null. One folder per document, by ruling. */
export function setNoteFolder(db: Db, orgId: string, uuid: unknown, folderId: unknown): void {
  ...
  if (target != null) {
    const exists = db.prepare("SELECT id FROM mindmerge_doc_folders WHERE id = ? AND org_id = ?").get(target, orgId);
    if (!exists) throw new Error("That folder does not exist.");
  }
  const changed = db
    .prepare("UPDATE mindmerge_docs SET folder_id = ?, updated_at = ? WHERE org_id = ? AND uuid = ?")
 ...
 *
 * The filename itself is dropped: a document is not its own folder. Depth is capped so a pathological
...
export function ensureFolderPath(
  ...
  for (const raw of parts.slice(0, MAX_DEPTH)) {
    ...
    const existing = db
      .prepare(
        parent == null
          ? "SELECT id FROM mindmerge_doc_folders WHERE org_id = ? AND name = ? AND parent_id IS NULL"
          : "SELECT id FROM mindmerge_doc_folders WHERE org_id = ? AND name = ? AND parent_id = ?"
      )
...
export function noteFolderSubtree(
  ...
  const marks = ids.map(() => "?").join(",");
  const notes = (db
    .prepare(`SELECT COUNT(*) AS n FROM mindmerge_docs WHERE org_id = ? AND folder_id IN (${marks})`)
    .get(orgId, ...ids) as { n: number }).n;
  // THE NUMBER THAT MADE THE TREE LOOK WRONG. This count includes archived documents; the tree's own
  // count (noteFolderCounts) excludes them. So a folder showing 0 could delete 1, and deleting one
  // document could drop the parent's total by two — which is exactly what Jason watched happen when he
  ...
  const archived = (db
    .prepare(`SELECT COUNT(*) AS n FROM mindmerge_docs WHERE org_id = ? AND folder_id IN (${marks}) AND archived_at IS NOT NULL`)
    .get(orgId, ...ids) as { n: number }).n;
  // DIRECT documents are the only ones "keep the notes" actually unfiles — the rest stay filed inside
  ...
  const directNotes = (db
    .prepare("SELECT COUNT(*) AS n FROM mindmerge_docs WHERE org_id = ? AND folder_id = ?")
