...
function SshPane({ keys, onReload }: { keys: VaultSecretMeta[]; onReload: () => void }) {
  ...
  const copyPass = (): void => {
    if (!current) return;
    setBusy(true);
    void api.read(current.uuid).then((f) => navigator.clipboard.writeText(f.extras?.passphrase ?? "")).then(() => setCopied(true)).catch(() => undefined).finally(() => setBusy(false));
  };
        ...
        {!art ? (
          ...
          <>
            ...
            <div className="vault-btnrow" style={{ marginTop: 12 }}>
              <button className="vault-btn" onClick={() => { void navigator.clipboard.writeText(current?.public_key ?? ""); onReload(); }}>Copy public key</button>
              <button className="vault-btn" onClick={() => void navigator.clipboard.writeText(art.fingerprint ?? "")}>Copy fingerprint</button>
            </div>
          </>
        )}
      </div>
...
function ImportRecords({ onImported }: { onImported: () => void }) {
  ...
  const commit = (): void => {
    ...
    void api.importZone(domain, parsed.records)
      .then((r) => { setMsg(`${r.imported} records imported for ${domain}.`); setParsed(null); setText(""); onImported(); })
      .catch((e: unknown) => setMsg(e instanceof Error ? e.message : String(e)))
      .finally(() => setBusy(false));
  };
