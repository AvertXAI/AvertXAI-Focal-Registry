...
export default function RenameModule() {
  ...
  const applyPreset = useCallback((p: RenamePresetRow) => {
    setSettings({
      ...
      sequenceStart: p.sequence_start ?? 1, sequencePad: p.sequence_pad ?? 3,
  ...
  useEffect(() => {
    const onP = (p: RenameProgress): void => {
      ...
      if (p.status !== "running") void refreshBatches();
    };
  ...
  return (
    <main className="view shown">
      <div className="wrap rename-shell">
          ...
          <div className="rn-split">
            <div className="rn-left">
              ...
              <div className="rn-row">
                <input className="rn-inp mono" style={{ maxWidth: 100 }} value={String(settings.sequenceStart)} onChange={(e) => set("sequenceStart", parseInt(e.target.value.replace(/\D/g, ""), 10) || 1)} />
                <select className="rn-inp" value={settings.sequencePad} onChange={(e) => set("sequencePad", Number(e.target.value))}>
                  <option value={3}>3 digits</option><option value={4}>4 digits</option><option value={2}>2 digits</option>
                </select>
              </div>
              <div className="rn-hint"><b>Each file type counts separately.</b> Stills run their own 001, 002, 003 and video runs its own. A RAW + JPEG pair shares one number.</div>
              ...
              <div className="rn-row">
                <input className="rn-inp mono" style={{ maxWidth: 150 }} value={settings.shootDate} placeholder="Date (YYYYMMDD)" onChange={(e) => set("shootDate", e.target.value)} />
                <input className="rn-inp" value={settings.customTag} placeholder="Custom tag (optional)" onChange={(e) => set("customTag", e.target.value)} />
              </div>
              <div className="rn-hint">Stored against every folder in this batch — not in the filename.</div>
              ...
              <div className="rn-row">
                ...
                <button className="rn-btn" onClick={async () => { const name = window.prompt("Save preset as:"); if (name) { await api.savePreset(name, settings); void refreshPresets(); } }}>Save</button>
              </div>
            </div>
        ...
        {tab === "history" && <History batches={renameOnly} onRevert={gotoRevert} onOpen={(p) => void api.openFolder(p)} />}
        {tab === "presets" && <Presets presets={presets} onApply={(p) => { applyPreset(p); setTab("new"); }} onDelete={async (id) => { await api.deletePreset(id); void refreshPresets(); }} />}
      </div>
    </main>
  );
}
      ...
      {batches.map((b) => (
        <div className="rn-batch" key={b.id}>
          ...
          <div className="rn-meta">
            ...
            <div><span>Shoot date</span><b>{b.shoot_date || "—"}</b></div><div><span>Prefix</span><b>{b.prefix_mode === "both" ? `${b.business_name}-${b.photographer_name}` : b.prefix_mode === "biz" ? b.business_name : b.photographer_name}</b></div>
