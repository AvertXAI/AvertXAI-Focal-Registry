...
1. **Read `CANON/FR-CANON-1.md`** — the scoped canon index for this repo — **and the four files it names**: `FR-FACTS-1.md`, `FR-RULES-1.md`, `FR-DECISIONS-1.md`, `FR-STATUS-1.md`.
...
**Stale** → `FR-CANON-1.md` records which real-canon versions it was derived from. If any file at `D:\dev\_source\AvertXAI-CANON\CANON PROJECT\` now carries a higher version number, the scoped set is stale. **Report it and stop.** Do not regenerate it yourself and do not proceed on a stale set for anything that touches a decision.
