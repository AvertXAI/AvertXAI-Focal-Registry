...
1. **List `CANON/` and read the highest-numbered `FR-CANON-*.md`** — the scoped canon index for this repo — **and the highest-numbered version of each of the four files it names** (`FR-FACTS-*.md`, `FR-RULES-*.md`, `FR-DECISIONS-*.md`, `FR-STATUS-*.md`). Rotation deletes the old numbers and the files rotate independently: mismatched numbers are normal, a missing lower-numbered file is not an error, and only an empty `CANON/` is a blocker.
...
**`PROJECT-CANON.md` is DEAD.** The scoped `CANON/FR-*` set supersedes it. If a copy is still present at the repo root, it is a leftover — do not read it, do not regenerate it, do not act on anything in it.

### 0.2 If the scoped canon set is missing or stale

**Missing** → stop, report, and do not start the task. The scoped set is **generated by Jason from real canon**, not by you.

**Stale** → the highest-numbered `FR-CANON-*.md` records which real-canon versions it was derived from. If any file at `D:\dev\_source\AvertXAI-CANON\CANON PROJECT\` now carries a higher version number, the scoped set is stale. **Report it and stop.** Do not regenerate it yourself and do not proceed on a stale set for anything that touches a decision.

**Scoped canon is authoritative FOR THIS REPO** — it is newer and written against verified code. Real canon stays authoritative for the operation. Where the two disagree, the scoped set wins here, and the disagreement goes in `CANON-UPDATES.md`.
...
- **The scoped `CANON/FR-*` set is GENERATED, never authored.** Do not hand-edit any of those five files and do not regenerate them — Jason generates them from real canon.
- **Silence is not permission.** When the scoped set says nothing about something, read real canon at `D:\dev\_source\AvertXAI-CANON\CANON PROJECT\` before concluding no rule exists. It is a filtered view, and a filter can drop something.
...
**System** — the section **renders only when a row exists in it.** An empty header would mean hardcoding a nav entry, which §3.5 bans. An absent System section is CORRECT, not a defect. GetScriptClips and Canon Distributor are **removed from this repo.**
