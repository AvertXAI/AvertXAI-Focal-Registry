/* Author: Jason Cruz | (c) 2026 AvertXAI | Proprietary — File: src/modules/mindmerge/NotesView.tsx */
// Secured Notes, copied into MindMerge as the AUTHORING stack (Jason 08-21-2026: MindMerge keeps BOTH
// storage shapes — the INGEST tables that index markdown found on disk stay untouched, and these
// authoring tables land beside them). Three list styles — Notes · Runbooks · Ideas — over ONE stored
// shape, a collapsible list, and the Milkdown editor. The layout decisions, in the order Jason made
// them:
...
// MODULE-LOCAL, NEVER THE VAULT'S (Jason 08-22-2026: "every module has to have its own --mm-btn-*
// relation to identify itself from other modules… if you keep vault-modal-width etc, your going to
// get the vaults secured notes ui crossed with mindmerges ui. config as data sop").
//
// These two came across the module line during the port, and the cost was exactly what he predicted:
// the vault's components emit .vault-modal / .vault-btn, and .vault-modal reads
// `max-width: var(--vault-modal-width)` — a token declared on `.vault-shell` (vault.css:7), an
...
/** The list's date stamp. It came from the vault's EntriesView, which is NOT part of this port, so
    the four lines live here rather than dragging a whole secrets table across the module line. */
 ...
 */
const listCache = new Map<string, MindMergeDocMeta[]>();
 ...
 * setting row it used to write (`notes.list_width`) is deliberately left in MINDMERGE_DEFAULTS:
 * removing a key that a stored database row still carries is a non-additive migration (§3.9), and an
 * ignored row costs nothing.
 *
...
/**
 * TWO SHELVES IN MINDMERGE, NOT THREE (Jason 08-22-2026: "i dont need runbooks on there").
 *
 * The `runbook` STYLE stays in the type, in TITLES and in BADGE, and that is deliberate rather than
 * half-done: STYLES is the only thing that draws the shelf chips, so dropping the row hides the
 * shelf outright. Removing the stored value would be a non-additive migration (§3.9) on rows that
 * already carry `kind='runbook'` — and any that exist must still render with a title and a badge
 * instead of blank. Hidden shelf, intact data. The Secured Vault keeps all three (08-22-2026:
 * "leave secured vault alone").
 */
const STYLES: [Style, string][] = [["note", "Notes"], ["snippet", "Ideas"]];
/** "Notes", not "Secured Notes" (Jason 08-22-2026: "i need the secured notes name change to just
 *  Notes") — the vault-era name came across with the port and never belonged on this side. */
const TITLES: Record<Style, string> = { note: "Notes", runbook: "Runbooks", snippet: "Ideas" };
const BADGE: Record<Style, [string, string]> = { note: ["N", "var(--mm-note-color)"], runbook: ["R", "var(--mm-strong-color)"], snippet: ["I", "var(--mc-accent-primary)"] };
...
export interface NotesViewProps {
   ...
   *
   * It was already being bumped and the folder TREE already listened to it — but MindMergeModule
   * hands it to the Sidebar only, so the note LIST never heard about a change made outside itself.
   * Drag a note into a folder and the tree's counts moved while the list still showed the note where
   * it had been. Ctrl+R "fixed" it because a reload remounts everything, which is precisely the tell.
   */
  reloadKey: number;
...
export default function NotesView({ settings, onSetting, onHelp, onImport, folderId, onNotesChanged, reloadKey, openUuid, onOpened }: NotesViewProps) {
  ...
  const [rows, setRows] = useState<MindMergeDocMeta[] | null>(() => listCache.get(cacheKey(style, "active", 0)) ?? null);
  const [current, setCurrent] = useState<MindMergeDoc | null>(null);
  ...
  // List collapse RETIRED 08-25-2026 (Jason: the « "needs to be on [the rail]") — the module's one
  // edgetab now collapses the folders sidebar instead. The stored notes.list_collapsed row is
  // deliberately ignored so an install that collapsed the list before today is not stuck shut.
   ...
   *
   * It fires ONLY when the title we sent was the untouched placeholder and the store came back with
    ...
    setAsk({
      ...
      body: (
        <>
          ...
          <p><b>Delete for good</b> erases them from MindMerge. <b>That cannot be undone.</b></p>
        </>
      ),
      label: `Archive ${n}`,
  ...
  const askDestroy = (): void => {
    ...
    setAsk({
      ...
      body: (
        <>
          <p><b>This cannot be undone.</b> The text is erased from MindMerge for good.</p>
          <p className="mm-hint">If you only want it out of the way, restore it and archive it instead.</p>
        </>
      ),
    ...
    setAsk({
      ...
      body: (
        <>
          ...
          <p className="mm-hint">Code blocks are untouched. The change shows in the editor immediately — look it over, then save.</p>
        </>
      ),
  ...
  useEffect(() => {
    ...
    const a = leftPaneRef.current?.querySelector<HTMLElement>(".mm-rawsrc");
    const b = rightPaneRef.current?.querySelector<HTMLElement>(".mm-milkdown");
    if (!a || !b) return;
    let lock = 0;
  ...
  return (
    <div className="mm-notewrap">
      <div className="mm-notelist" style={{ width: LIST_WIDTH }}>
        ...
        <div className="mm-nltop">
          <b className="mm-nltitle">{TITLES[style]}</b>
          {shelf === "active" && (
            <button className="mm-collapse import" title={`Import ${TITLES[style].toLowerCase()}`} onClick={onImport}>
              <span aria-hidden="true">⭳</span> Import
            </button>
          )}
          {/* chevron removed 08-20-2026 — this list collapses from the .edgetab on its right edge */}
        </div>
        <div className="mm-nlstyles">
          {STYLES.map(([s, label]) => (
        ...
        <div className="mm-nlsearch" style={{ justifyContent: "flex-end" }}>
          ...
          {shelf === "active" ? (
            <>
              ...
              <button className="mm-btn primary sm" onClick={newNote}>+ New</button>
            </>
          ) : (
            <>
              <button className="mm-iconbtn" title={picked.size === 0 ? "Tick one or more first" : `Restore ${picked.size}`}
                disabled={picked.size === 0} onClick={askRestore}>↺</button>
              <button className={`mm-iconbtn trash${picked.size > 0 ? " armed" : ""}`}
        ...
        {outcome && (
          <div className="mm-hint" style={{ padding: "4px 10px 8px", color: "var(--mm-strong-color)" }}>
            {outcome}{" "}
        ...
        >
          {rows === null ? (
            ...
            folderId === -1 ? (
              ...
              // mount with "Nothing here yet", which read as the store losing the notes (Jason,
              // 08-14: "all my notes went missing"). Say where they actually are.
              <div className="mm-state">Unfiled is empty — your filed notes are in the folders on the left.<div><button className="mm-btn" onClick={newNote}>Write a new one</button></div></div>
            ) : (
              <div className="mm-state">Nothing here yet.<div><button className="mm-btn" onClick={newNote}>Write the first one</button></div></div>
            )
          ) : (
            shown.map((n) => (
              <div
                ...
                // The drag TYPE string is carried over verbatim: it is a private handshake with
                // NoteFolderRail, and both ends must agree — renaming it here alone breaks the drop.
                draggable
                onDragStart={(e) => {
                  e.dataTransfer.setData("text/mindmerge-doc", n.uuid);
              ...
              >
                ...
                />
                <span className="t"><span className="mm-ticon" style={{ background: badgeColor }}>{b}</span> {n.title}</span>
          ...
          {loadingMore && <div className="mm-hint" style={{ padding: "8px 12px" }}>Loading more…</div>}
          {!loadingMore && truncated && (
            <div className="mm-hint" style={{ padding: "8px 12px" }}>
      ...
      <div className="mm-noteedit">
        {/* THE TOOLBAR — Jason's mockup v4 (08-16-2026), button for button: undo/redo · text
            style · marks · link/image/table · lists · blocks · Tidy · the mode segment · help.
            (The @🔑 credential-reference button was vault-only and does not cross into MindMerge —
            08-21-2026 port ruling.) Every button goes through act(), which picks the engine the
            mode mounts: Milkdown commands in Editor (formatting applies as you type — his ruling),
            ...
            pasting. */}
        <div className="mm-edbar">
          <button className="mm-tbtn" data-tip="Undo — Ctrl+Z" disabled={!current || mode === "preview"} onClick={() => act("undo")}>↺</button>
          <button className="mm-tbtn" data-tip="Redo — Ctrl+Y" disabled={!current || mode === "preview"} onClick={() => act("redo")}>↻</button>
          <span className="mm-tsep" />
          <div className="mm-pselect">
            <button className="mm-tbtn wide" data-tip="Text style" disabled={!current || mode === "preview"}
              onClick={(e) => { e.stopPropagation(); setPmenuOpen((o) => !o); }}>
              {pstyle} <span style={{ fontSize: 10 }}>▾</span>
            </button>
            {pmenuOpen && (
              <div className="mm-pmenu">
                {([
          ...
          <span className="mm-tsep" />
          <button className="mm-tbtn" data-tip="Bold — Ctrl+B" disabled={!current || mode === "preview"} onClick={() => act("bold")}><b>B</b></button>
          <button className="mm-tbtn" data-tip="Italic — Ctrl+I" disabled={!current || mode === "preview"} onClick={() => act("italic")}><i>I</i></button>
          <button className="mm-tbtn" data-tip="Strikethrough" disabled={!current || mode === "preview"} onClick={() => act("strike")}><s>S</s></button>
          <button className="mm-tbtn mono" data-tip="Inline code" disabled={!current || mode === "preview"} onClick={() => act("code")}>{"</>"}</button>
          <span className="mm-tsep" />
          <button className="mm-tbtn" data-tip="Link — Ctrl+K" disabled={!current || mode === "preview"} onClick={() => act("link")}>🔗</button>
          <button className="mm-tbtn" data-tip={mode === "editor" ? "Paste a screenshot straight into the document" : "Image — paste a screenshot"}
            disabled={!current || mode === "preview" || mode === "editor"} onClick={() => act("image")}>🖼</button>
          <button className="mm-tbtn" data-tip="Table" disabled={!current || mode === "preview"} onClick={() => act("table")}>⊞</button>
          <span className="mm-tsep" />
          <button className="mm-tbtn" data-tip="Bullet list" disabled={!current || mode === "preview"} onClick={() => act("bullet")}>•≡</button>
          <button className="mm-tbtn" data-tip="Numbered list — in Runbooks these become steps" disabled={!current || mode === "preview"} onClick={() => act("ordered")}>1≡</button>
          <button className="mm-tbtn" data-tip="Checklist" disabled={!current || mode === "preview"} onClick={() => act("task")}>☑</button>
          <span className="mm-tsep" />
          <button className="mm-tbtn" data-tip="Block quote" disabled={!current || mode === "preview"} onClick={() => act("quote")}>❝</button>
          <button className="mm-tbtn mono" data-tip="Code block" disabled={!current || mode === "preview"} onClick={() => act("codeblock")}>{"{ }"}</button>
          <button className="mm-tbtn" data-tip="Divider line" disabled={!current || mode === "preview"} onClick={() => act("hr")}>—</button>
          <span className="mm-tsep" />
          <button className={`mm-btn sm${needsTidy ? " tidyhot" : ""}`}
          ...
          <div className="mm-seg mm-modeseg">
          ...
          <button className="mm-help" data-tip="Markdown help" onClick={onHelp}>?</button>
        </div>

        {error && <div className="mm-state error">{error}</div>}

        {current === null ? (
          <div className="mm-state">Pick a {style} to open it, or start a new one.</div>
        ) : (
          <div className="mm-notesplit">
            ...
            {mode !== "preview" && (
              <div className="mm-pane left" ref={leftPaneRef}>
                ...
                <div className="mm-panehdr">
                  <span className="mm-editdot" /> {mode === "editor" ? "Editor" : "Raw"}
                  <input className="mm-notetitle inhdr" value={title} onChange={(e) => { setTitle(e.target.value); setDirty(true); }} onBlur={save} placeholder="Title" />
                </div>
                {mode !== "editor" ? (
                  <textarea
                    ref={rawRef}
                ...
                <div className="mm-savebar">
                  <span className="mm-hint">{dirty ? "Editing…" : "Saved."}</span>
                  {/* Brief, and only after the idle save — a permanent "Saved" is wallpaper. */}
                  <span key={justSaved} className={`mm-savedflash${showSaved ? " on" : ""}`}>✓ Saved</span>
                </div>
                <div className="mm-btnrow" style={{ marginTop: 8 }}>
                  <button className="mm-btn" disabled={!dirty} onClick={save}>Save now</button>
                </div>
              </div>
            )}
            ...
            {mode === "split" && (
              <div className="mm-pane prev" ref={rightPaneRef}>
                <div className="mm-panehdr">
                  <span className="mm-editdot" /> Editor
                  <span className="mm-notetitle inhdr ro" title={title}>{title || "Untitled"}</span>
                </div>
                <MilkdownEditor
                  ref={editor}
            ...
            {mode === "preview" && (
              ...
              >
                ...
                {previewIsRun ? (
                  <RunMode body={draft} title={title} />
                ) : (
                  <div className="mm-preview" ref={prevScrollRef}>
                    {sourceText.trim() ? (
                      ...
                      <Markdown body={sourceText} />
                    ) : (
                      <div className="mm-state">Nothing written yet.</div>
                    )}
                  </div>
                )}
              </div>
