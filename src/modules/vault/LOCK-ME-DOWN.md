...
- **File:** `vault_<org_id>.locked.db` (under Electron `userData`; one per org, routed by the platform registry)
...
- **Key Management (Envelope Encryption):** a 32-byte random secret is generated on first run, encrypted via the OS native keychain (Electron's `safeStorage` / DPAPI on Windows), and stored alongside the DB as `vault_<org_id>.key`. The plaintext secret never touches the repo, the DB, or `app_settings`.
