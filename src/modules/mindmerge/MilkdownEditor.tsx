...
/* File: src/modules/mindmerge/MilkdownEditor.tsx */
...
// which mindmerge.css already styles for the preview pane. The editor reuses those rules rather than
...
import { isMindMergeSrc, resolveAttachmentSrc } from "./attachmentSrc";
 ...
 * deliberate — a bad paste must never take the docs screen down — but silent-AND-unrecorded is
 * how the same bug got screenshotted three times. MindMerge has no event log of its own, so this
 * routes them to the console under a stamped prefix you can filter on. Message and stack only,
 * NEVER document content. The reporter itself may never throw a second failure into the path it
 * is reporting on.
 */
function tell(level: "warn" | "error", message: string, err?: unknown): void {
  if (level === "error") console.error(`[mindmerge:renderer] ${message}`, err ?? "");
  else console.warn(`[mindmerge:renderer] ${message}`, err ?? "");
}
...
const MilkdownEditor = forwardRef<MilkdownHandle, MilkdownEditorProps>(function MilkdownEditor(
      ...
      run: (action) => {
        ...
        try {
          if (typeof action === "object") {
            if ("insert" in action) {
              // Markdown text handed in by the toolbar. `true` = inline, so it lands in the current
          ...
          /* a failed command must not take the docs screen down — but it must be on record */
    ...
    void Editor.make()
      .config((ctx) => {
        ...
        ctx.update(editorViewOptionsCtx, (prev) => ({
           ...
           */
          /**
           * CLICK AN IMAGE, SEE THE IMAGE (Jason 08-15-2026). Images render capped by CSS so they
           * cannot blast the pane; the full-size view is one click away, in a modal the parent
           * owns. `direct` only — a click on text that merely lands near an image must not hijack
           * the caret. Returning true stops ProseMirror turning the click into a node selection,
           * which is the right trade: selecting an image you cannot really see is not a thing
           * anyone wants; deleting one still works from either side with the keyboard.
           */
           ...
           *
           * WHAT WAS ACTUALLY HAPPENING, from diffing his source against the row that was stored:
          ...
          handlePaste: (view, event) => {
             ...
             * again. Now the BYTES go to MindMerge's attachment store and the document gets one
             * readable line: ![pasted-image.png](mindmerge://<uuid>). The insert happens when the
             * save lands (a beat later, not blocking the paste), at the selection that was active.
             */
            ...
            if (imageFile) {
               ...
               * image node lands NOW; its real mindmerge:// id is swapped in when the save
               * returns; a refused save removes the node rather than leaving a corpse.
               */
              const { state } = view;
              const imgType = state.schema.nodes.image;
              const paraType = state.schema.nodes.paragraph;
              if (!imgType) return true;
              const nonce = `mindmerge-pending:${Math.random().toString(36).slice(2)}`;
              ...
              reader.onload = () => {
                ...
                void mindmergeApi()
                  ...
                  .then((meta) => finish(`mindmerge://${meta.uuid}`))
                  .catch((err: unknown) => {
                    // Refused (not wired yet, oversize): the paste is lost, never the document —
                    // and SAID SO.
                    tell("error", "Editor: a pasted image could not be saved to MindMerge — the paste was removed", err);
                    finish(null);
                  })
         ...
         * one: Tidy, the Split raw-sync, switching documents — could fire one final time AFTER the
         * parent had already moved to newer text, handing it the OLD document. That un-tidied
         * the document (the damage "coming back"), could revert raw-pane edits, and could bleed one
         * document's text into another on a fast switch. The guard makes a torn-down instance mute:
      ...
      .catch((err: unknown) => {
        // A failed editor must not take the docs screen down with it — but an empty pane with
  ...
  /**
   * RESOLVE mindmerge:// IMAGES IN THE EDITOR'S DOM. The document node keeps `mindmerge://<uuid>`
   * as its src — that is what serialises back into the markdown — but a browser cannot load that
   * scheme, so the on-screen <img> element gets its src swapped to the resolved data URL. A DOM
   * swap, not a node change: the serializer reads node attrs, never the DOM, so the stored text is
   * untouched. ProseMirror may redraw the element on nearby edits and re-stamp mindmerge://; the
  ...
  useEffect(() => {
    ...
    const sweep = (): void => {
      root.querySelectorAll<HTMLImageElement>("img").forEach((img) => {
        ...
        if (raw?.startsWith("mindmerge-pending:")) {
          const url = pendingPreview.get(raw);
          if (url && img.src !== url) img.src = url;
          return;
        }
        if (!isMindMergeSrc(raw)) return;
        void resolveAttachmentSrc(raw as string)
          ...
          .catch((err: unknown) => {
            // Unavailable or gone: the broken-image glyph is the honest state here. Logged ONCE
            // per reference — the sweep re-fires on every redraw and an attachment store that
            // refuses every read is not a flood.
            if (!toldSrcFailures.has(raw as string)) {
              toldSrcFailures.add(raw as string);
              tell("warn", "Editor: a pasted image could not be loaded from MindMerge", err);
            }
          });
      });
    };
  ...
  return <div className="mm-milkdown" ref={host} />;
});
