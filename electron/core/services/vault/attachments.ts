...
// Description: Pasted-image attachments for Secured Notes — the bytes live in the encrypted vault
//              file, the note body carries only `![name](vault://<uuid>)` (Jason 08-16-2026,
//              against the base64 wall the data-URL paste produced; the Joplin `:/id` shape,
//              vault-flavoured). Images only for now — the 08-12 ruling covers what a user pastes
//              into their own note; widening to PDFs is its own decision when it is asked for.
//              Transport is base64 because the value crosses the IPC bridge as JSON; at rest it is
//              a real BLOB, not text.
// License: Proprietary / Unauthorized copying of this file is strictly prohibited
// File: electron/core/services/vault/attachments.ts
//------------------------------------------------------------
...
export function saveAttachment(
  ...
  const at = nowIso();
  db.prepare(
    `INSERT INTO vault_attachments (uuid, org_id, name, mime, byte_count, bytes, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?)`
...
export function getAttachment(
  ...
  const row = db
    .prepare("SELECT name, mime, bytes FROM vault_attachments WHERE org_id = ? AND uuid = ?")
    .get(orgId, uuid) as { name: string; mime: string; bytes: Buffer } | undefined;
  if (!row) throw new Error("That image is no longer in the vault.");
