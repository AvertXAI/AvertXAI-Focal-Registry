# FR-DECISIONS-6.md — settled choices for Focal Registry

Current state, one line each. Do not reopen unless Jason says so.
*(Derived 2026-08-17 from DECISIONS-53 — Scan Notes rulings absorbed, Scan charter amended. Supersedes FR-DECISIONS-5 — delete it after placement.)*
...
- **Sanctioned writes outside the application's own data folder are SIX (amended 2026-08-17):** Scan's report · Rename's destination copies · Migrate's bundle export · Scan's extraction · Scan Notes' app-owned tree (drive + local `Documents\Focal Registry\Scan Notes\`) · Scan's queued FOLDER renames. Migrate Phase 2 install would be a confirm-gated seventh. User media is NEVER written or overwritten — the folder rename is the only touch on user content.
...
## Scan
- Reports on drives. Stills, video, **and audio** equally. **CHARTER AMENDED 2026-08-17:** Scan renames scanned FOLDERS on disk via the Scan Notes pending queue (immediate when connected, queued and auto-applied on reconnect keyed to volume serial); never renames files, never deletes; Rename keeps file-level renaming. The header copy "Read-only; never renames or deletes" changes in the build.
...
## Scan Notes (RULED 2026-08-17 — mockup v4 approved)
- The Vault's Secured Notes stack COPIED (Milkdown types, Markdoc renders, autosave, import, search), encryption removed, plaintext in the SHARED org database — own tables: notes + append-only folder-name history (no cap; old AND new names both queryable).
- Two files per scanned folder: generated report markdown (read-only, never touched) + a user notes file; **+ Add Note** creates and saves into the selected folder automatically — no dialog.
- Folder History card locked above the editor; template `date | time | [Capture range min → max] | [Folder Name change: old -> new]` — card shows the LATEST change; the full chain lives in the history table and Updated Notes.
- Dual tree, **dumb-and-total sync**: every app-owned file overwritten every sync; folder renames applied exactly once, never duplicated; conflicts skip-and-log with a visible result.
- Auto-sync on drive connect keyed to volume serial; informational toast (12 seconds); a refresh control highlights with a count when pending work exists.
- **Updated Notes tab:** append-only feed — note created/edited · folder renamed · scan completed · sync applied — unseen-count badge on the tab; persists across every drive connection and scan.
- Desktop shortcut targets the local tree; one-click create warns and refuses a duplicate.
- **Media browse:** dual-purpose View media / Scan Notes toggle in the SAME window — image lightbox, H264 player modal, RAW via exifr embedded preview. Extends the 2026-07-26 viewer relaxation; playback only, no extraction, no downloads — NOT a god-mode revival.
...
- Forward-only: populates from NEW scans; no retroactive generation.
