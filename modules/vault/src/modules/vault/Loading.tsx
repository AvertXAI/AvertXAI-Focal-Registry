...
//
// The ring honours prefers-reduced-motion through .vault-spinner's existing media query; the text
...
export default function Loading({ message, done, total, unit = "files", compact = false }: LoadingProps) {
  ...
  return (
    <div className={`vault-loading${compact ? " compact" : ""}`} role="status" aria-live="polite">
      <div className="vault-loadring" />
      <div className="vault-loadmsg">{message}</div>
      {counted && (
        <div className="vault-loadcount">
