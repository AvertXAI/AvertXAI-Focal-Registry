...
/** How many secrets sit directly in each folder — the counts the tree shows. */
export function folderCounts(db: Db, orgId: string): Record<number, number> {
  const rows = db
    .prepare("SELECT folder_id, COUNT(*) AS n FROM vault_secrets WHERE org_id = ? AND archived_at IS NULL AND folder_id IS NOT NULL GROUP BY folder_id")
    .all(orgId) as { folder_id: number; n: number }[];
