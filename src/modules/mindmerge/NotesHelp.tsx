...
//
// MINDMERGE COPY (Phase 4, 08-22-2026). Copied verbatim from src/modules/vault/NotesHelp.tsx with
// the mechanical renames only (vault- classes -> mm-, vault:// -> mindmerge://, "the vault" in
// user-visible prose -> MindMerge). ONE row is STRIPPED, not renamed: the "Vault reference"
// @[[vault:…]] chip row — MindMerge has no credential store (markdown.tsx keeps that rewrite
// deliberately absent), so a help row describing Reveal/Copy password chips would document a
// feature this module must not have. The strip is reported in the Phase 4 lane report.
...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    rows: [
      ...
      {
        k: "image picture screenshot ![ alt mindmerge paste",
        syn: "![alt text](image.jpg)",
        name: "Image",
        desc: <>Same as a link, with a <code>!</code> in front. Paste a screenshot and this is written FOR you as <code>![name](mindmerge://…)</code> — the picture lives in MindMerge, the text stays one short line.</>,
...
function MRow({ r }: { r: Row }) {
  return (
    <div className="mm-mrow">
      <div className="mm-msyn">{r.syn}</div>
      <div className="mm-mwhat">
        <div className="mname">{r.name}</div>
        <div className="mdesc">{r.desc}</div>
      </div>
      <div className="mm-mout">{r.out}</div>
    </div>
  );
}
...
export function NotesHelpModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="mm-modalback" onClick={onClose}>
      <div className="mm-hmodal" ref={boxRef} style={size ? { height: size.h } : undefined} onClick={(e) => e.stopPropagation()}>
        <div className="mm-hmhead">
          <div className="mm-hmtoprow">
            <div>
              <h3>Markdown, in one card</h3>
              <div className="mm-hmsub">
            ...
            <button className="mm-hmclose" title="Close" onClick={onClose}>✕</button>
          </div>
          <input
            className="mm-hmsearch"
            placeholder="🔍  Search the guide — try 'table', 'bold', or 'checkbox'…"
            value={q}
            onChange={(e) => setQ(e.target.value)}
          />
        </div>
        <div className="mm-hmtabs">
          {SECTIONS.map((s) => (
            <button key={s.id} className={`mm-htab${!query && tab === s.id ? " on" : ""}`} onClick={() => { setQ(""); setTab(s.id); }}>
              {s.label}
            </button>
          ))}
        </div>
        <div className={`mm-hmbody${query || size?.clamped ? " scroll" : ""}`}>
          {query ? (
            hits.length === 0 ? (
              <div className="mm-nores">No matches — try a shorter word, like "list" or "code".</div>
            ) : (
              hits.map(({ s, rows }) => rows.map((r) => <MRow key={`${s.id}:${r.name}`} r={r} />))
            )
          ) : (
            <>
              {active.note && <div className="mm-hsecnote">{active.note}</div>}
              {active.rows.map((r) => <MRow key={r.name} r={r} />)}
            </>
          )}
        </div>
        <div className="mm-hmfoot">
          <span className="ref">Full reference: <span>markdownguide.org/cheat-sheet</span></span>
          <button className="mm-btn primary" onClick={onClose}>Got it</button>
        </div>
      </div>
    </div>
  );
