...
// Description: The raw-pane formatting engine — one pure function turning a toolbar action plus a
//              textarea selection into ONE contiguous replacement, so the native undo stack keeps
//              working. Drives Raw mode and Split's raw pane; the WYSIWYG uses Milkdown's own
//              commands instead.
// License: Proprietary / Unauthorized copying of this file is strictly prohibited
// File: src/modules/mindmerge/rawFormat.ts
//------------------------------------------------------------
 ...
 *
 * The vault build carried one more object action here — a credential-chip insert with a "Label"
 * placeholder to select. MindMerge has no credential store, so that action is deliberately absent;
 * heading is the only object-shaped action, and anything else object-shaped falls through to null
 * rather than being guessed at.
 */
export function rawEdit(action: EditorAction, text: string, s: number, e: number): RawEdit | null {
  if (typeof action === "object" && "heading" in action) {
