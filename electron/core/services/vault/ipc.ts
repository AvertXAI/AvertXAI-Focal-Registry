...
export function registerVaultIpc(): void {
  ...
  /**
   * ---- The clipboard funnel (Tier-1 fix 1). Every copy in the module lands here so the
   * "Clear clipboard after N seconds" setting actually happens — see clipboard.ts for the story.
   * UNGATED like vault:listEvents, and for a matching reason: the text travels FROM the renderer
   * to the system clipboard; nothing is read out of the vault, so there is nothing for the lock
   * to protect — and the one copy that must work while locked (the error log's "Copy for support")
   * would break behind the gate. The setting is read live on every call, never cached.
   */
  ...
  safeHandle("vault:copyText", async (_e, text: unknown) => {
    const { db, orgId } = await vaultCtx();
    copyWithClear(
      typeof text === "string" ? text : String(text ?? ""),
      vaultSettings.getNumber(db, orgId, "clipboard.clear_seconds"),
      CLIP,
      CLOCK
    );
    return true;
