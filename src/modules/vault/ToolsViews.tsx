...
import { vaultApi, type VaultAccessRow, type VaultHealthReport } from "./vaultApi";
import GeneratorPanel from "./GeneratorPanel";
import { shortDate } from "./EntriesView";

// ---------------------------------------------------------------- generator
// The generator itself moved to GeneratorPanel.tsx so the entry form renders the SAME one, with
// all six modes and every option, rather than a second weaker copy. This tab is now the panel in
// a card — which is all it ever was.
export function GeneratorView({ settings, onSetting }: { settings: Record<string, string>; onSetting: (k: string, v: string) => void }) {
  return (
    <div className="vault-card">
      ...
      <GeneratorPanel settings={settings} onSetting={onSetting} />
    </div>
  );
}
