...
export default function VaultModule() {
  ...
  /** A counter the folder tree watches so counts re-read after a write. The folder SELECTION moved
   *  below setSetting — it persists as a row now (08-14-2026). */
  ...
  /**
   * Secured Notes folder selection — PERSISTED as a row like every other view state (§3.8).
   * Defaults to UNFILED, not "everything" (Jason 08-12-2026): with 4,000 notes imported, opening on
   * the whole corpus is the load the windowing work exists to avoid.
   *
   * It was plain useState, so ANY navigation away unmounted the module and reset it to Unfiled —
   * and because the 08-12 rule hides Unfiled's rail row when empty, a fully-filed corpus came back
   * to an INVISIBLE selection over an empty list, which read as data loss (Jason 08-14: "all my
   * notes went missing"). The row remembers where you were; -1 stays the first-ever default.
   */
  const noteFolder = useMemo(() => {
    const n = Number(settings["notes.folder_selected"]);
    return Number.isInteger(n) ? n : -1;
  }, [settings]);
  const setNoteFolder = useCallback((id: number): void => setSetting("notes.folder_selected", String(id)), [setSetting]);
  /** STABLE on purpose — as an inline arrow this sat in NotesView's openUuid effect deps, so every
   *  VaultModule re-render re-ran the search-open effect while a note was pending (part of the
   *  08-14 "opens and closes in a loop" report). */
  const onNoteOpened = useCallback((): void => setPendingNote(null), []);
      ...
      <div className="vault-main-col">
        ...
        <div className="vault-shellbody">
          ...
          <div className="vault-body">
            {section === "errors" && <ErrorBoundary surface="Activity & errors"><ErrorsView settings={settings} onSetting={setSetting} onCounts={setErrorCount} /></ErrorBoundary>}
