...
export interface VaultApi {
   ...
   *  exact value when the timer fires. Ungated: text travels INTO the clipboard; nothing is read
   *  out of the vault, and the error log's "Copy for support" must work while locked. */
  ...
  /** An empty approved list is a NO-OP with a sentence in `message` — never a wipe (Tier-1 fix 3). */
  importZone: (domain: string, records: VaultZoneRecord[]) => Promise<{ imported: number; message?: string }>;
