...
export default function ConfirmModal({
  ...
  return (
    <div className="mm-modalback" onClick={blocked === undefined ? onClose : undefined}>
      <div ref={root} className="mm-modal mm-confirm" role="alertdialog" aria-modal="true" onClick={(e) => e.stopPropagation()}>
        <h3>{title}</h3>
        <div className="mm-confirmbody">{body}</div>
        {nag && <div className="mm-confirmnag" role="alert">{nag}</div>}
        <div className="mm-modalacts">
          <button ref={cancelRef} className="mm-btn" onClick={onClose}>{cancelLabel}</button>
          ...
          {secondary && (
            <button
              className={`mm-btn ${secondary.danger ? "danger" : ""}`}
          ...
          <button
            ref={goRef}
            className={`mm-btn ${danger ? "danger solid" : "primary"}${blocked ? " held" : ""}`}
