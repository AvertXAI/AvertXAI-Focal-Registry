...
export function VaultChip({ label, secrets }: { label: string; secrets: VaultSecretMeta[] }) {
  ...
  const copy = (): void => {
    setBusy(true);
    void api.read(hit.uuid).then((f) => api.copyText(f.value)).then(() => setCopied(true))
...
function CodeBlock({ content, language }: { content?: string; language?: string }) {
  ...
  return (
    <div className="vault-cblock">
      <div className="cb">
        <span>{lang || "text"}</span>
        <button type="button" onClick={() => { void vaultApi().copyText(text); setCopied(true); }}>
          {copied ? "Copied" : "Copy"}
        </button>
      </div>
      {tokens === null ? (
            ...
            {s.commands.map((c, j) => (
              <div key={j} className="vault-cmd">
                ...
                <button type="button" className="vault-btn"
                  onClick={() => { void vaultApi().copyText(c); setCopiedKey(`${i}-${j}`); }}>
