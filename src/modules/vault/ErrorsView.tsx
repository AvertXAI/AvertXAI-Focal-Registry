...
export default function ErrorsView({ settings, onSetting, onCounts }: { settings: Record<string, string>; onSetting: (k: string, v: string) => void;
  /** Mirrors the error count up to the module's ⚠ badge. The badge's own 2-minute poll was up to
   *  120 seconds stale exactly when the user was LOOKING at this page — 6 on the chip against 12
   *  in the header (Jason 08-14). While this view is mounted, its 3-second load is the truth. */
  onCounts?: (errors: number) => void }) {
  const api = vaultApi();
  ...
  useEffect(() => {
    if (rows !== null) onCounts?.(counts.error);
  }, [rows, counts, onCounts]);
