...
// File: electron/core/services/runbook-shredder/db.ts
//------------------------------------------------------------
...
// Open (or reuse) this module's own DB file. PLAIN mode — no cipherKey — because runbook content
...
export function openShredderDb(orgId: string, baseDir: string): Db {
  const name = `runbook-shredder:${orgId}`;
  const existing = registry.get(name);
  if (existing) return existing;
  const db = new Database(path.join(baseDir, `runbook-shredder_${orgId}.db`));
...
export function createSchema(db: Db): void {
  createTable(db, "runbooks", [
    "runbook_id TEXT UNIQUE", // from frontmatter `id`; NULL allowed (quarantined rows have none)
    "title TEXT",
    "type TEXT",
    "status TEXT",
    "severity TEXT",
  ...
  db.exec("CREATE UNIQUE INDEX IF NOT EXISTS runbook_tags_uniq ON runbook_tags(runbook_id, tag_id);");

  // POINTERS ONLY — vault_pointer is a locator like "hetzner/avert-core-01/ssh", never a secret value.
  createTable(db, "runbook_secret_refs", [
    "runbook_id INTEGER NOT NULL REFERENCES runbooks(id) ON DELETE CASCADE",
    "ref_key TEXT NOT NULL",
    "vault_pointer TEXT NOT NULL",
  ]);

  // FTS5 external-content over runbooks(title, body_md, tags_flat) keyed to runbooks.id.
  // Virtual table — exempt from the std-columns wrapper by design.
  db.exec(`CREATE VIRTUAL TABLE IF NOT EXISTS runbooks_fts USING fts5(
    title, body_md, tags_flat,
    content='runbooks', content_rowid='id'
  );`);
  ...
  db.exec(`
    CREATE TRIGGER IF NOT EXISTS runbooks_ai AFTER INSERT ON runbooks BEGIN
      INSERT INTO runbooks_fts(rowid, title, body_md, tags_flat)
        VALUES (new.id, new.title, new.body_md, new.tags_flat);
    END;
    CREATE TRIGGER IF NOT EXISTS runbooks_ad AFTER DELETE ON runbooks BEGIN
      INSERT INTO runbooks_fts(runbooks_fts, rowid, title, body_md, tags_flat)
