...
export function ensureVaultSchema(db: Db): void {
  ...
  /**
   * PASTED-IMAGE ATTACHMENTS (Jason 08-16-2026: "it spits out a garage of text… joplin does this:
   * ![name](:/id)"). The bytes live HERE, in the encrypted file, and the note body carries only a
   * short reference — `![name](vault://<uuid>)` — so the Raw view shows one readable line instead
   * of a 200,000-character base64 wall. Images ruled IN for Secured Notes 08-12-2026 (the §4.1
   * "no pixels" rule binds Scan/Rename and the archive; a screenshot a user pastes into their own
   * encrypted note is content, not an archive artefact).
   *
   * No foreign key to the note ON PURPOSE, matching every soft reference in this file: a note can
   * be deleted while its image is still referenced from a version of the text somewhere, and a
   * dangling reference renders as a missing image, never a crash. Orphan sweep is a later,
   * deliberate maintenance path — compaction reclaims the space when it runs.
   */
  createTable(db, "vault_attachments", [
    "org_id TEXT NOT NULL",
